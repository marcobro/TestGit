...
class Kiwoom(QAxWidget):
    ...
    def _opt10081(self, rqname, trcode):
        ...
        for i in range(data_cnt):
            jong = self._comm_get_data(trcode, "", rqname, 0, "종목코드")
...
if __name__ == "__main__":
    ...
    con = sqlite3.connect("c:/Users/marco/etf.db")
    
    for i in range(len(code_list)):
        jong = code_list[i]
        # opt10001 TR 요청(종목정보)
        kiwoom.ohlcv = {'jong':[], 'jong_nm':[], 'listed_shares':[], 'cap':[], 'cap_rate':[], 'foregin_rate':[], 'per':[], 'eps':[], 'roe':[], 'pbr':[], 'ev':[], 'bps':[], 'sales':[], 'op_profit':[], 'net_income':[], 'traded_shares':[], 'traded_shares_rate':[]}
        # opt10081 TR 요청
        kiwoom.set_input_value("종목코드", jong)
        kiwoom.comm_rq_data("opt10001_req", "opt10001", 0, "0101")
        time.sleep(TR_REQ_TIME_INTERVAL)
        
        print(kiwoom.ohlcv)
        
        df = pd.DataFrame(kiwoom.ohlcv, columns=['jong','jong_nm','listed_shares', 'cap', 'cap_rate', 'foregin_rate', 'per', 'eps','roe','pbr','ev','bps','sales','op_profit','net_income','traded_shares','traded_shares_rate' ])    
        df.to_sql('INFO_STOCK', con, if_exists='append', index=False)    
    
        kiwoom.ohlcv = {'wkdate': [], 'jong':[], 'start_price': [], 'high_price': [], 'low_price': [], 'close_price': [], 'volume': []}

        # opt10081 TR 요청
        kiwoom.set_input_value("종목코드", jong)
        kiwoom.set_input_value("기준일자", "20190810")
        ...
        df = pd.DataFrame(kiwoom.ohlcv, columns=['wkdate','jong','start_price', 'high_price', 'low_price', 'close_price', 'volume'])

        df.to_sql('CLOSE_PRICE', con, if_exists='append',index=False)